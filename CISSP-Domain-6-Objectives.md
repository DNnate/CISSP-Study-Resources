...
- 6.2.3 Log reviews
    ...
    - **Network flow** (NetFlow) logs are particularly useful when investigating security incidents. NetFlow records contain an entry for every network communication session that took place on a network and can be compared to a list of known malicious hosts.
