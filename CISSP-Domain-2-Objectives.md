...
- 📁**Understanding System Tiers in Reference Architecture**
    - 🎈Tier 0: This tier represents client or endpoint devices that interact with applications and services. Examples include:     Desktops, Laptops, Mobile devices (smartphones, tablets), IoT devices. These devices are considered the entry points of data interaction in the system.
...
- 2.6.1 Data states (e.g., in use, in transit, at rest): **🥇Strong encryption is the only method that would protect data at all stages, in transit, rest, or even at the end of the life cycle.**
  - The three data states are at rest, in transit, and in use
    ...
    - Network-level encryption protects data in transit. 
      ...
      - 🥑**Link Encryption**:  link encryption refers to a method of securing data as it travels across a communication link.
          - If the transmission path looks something like this: workstation → switch 1 → router 1 → router 2 → router 3 → switch 2 → server and link encryption is being used, the data will be decrypted in three of the networking devices (perhaps 5, depending on the type of switches and the exact encryption mechanisms). Sticking with just the basics, for link encryption, the data would be encrypted by the workstation and then decrypted by router 1. Router 1 then encrypts the data and sends it to router 2, which decrypts the data. This is repeated at router 3. Then the final decryption is done at the server. workstation (E) → switch 1 → router 1 (D & E) → router 2 (D & E) → router 3 (D & E) → switch 2 → server (D) where E = encrypt & D = decrypt
          - As a result, the data is in the clear in the routers. If they are compromised, the data is in plain text.
          - The time it takes to perform encryption and decryption is greatly reduced with today's technology. However, there is still the possibility that it could cause a noticeable delay, depending on the application.
          - Although link encryption secures the data at each link in the network, the data is not encrypted when it is being processed at intermediate nodes. This contrasts with end-to-end encryption, where data is encrypted on the sender's device and only decrypted on the recipient's device.
          - Link encryption is commonly used in scenarios where multiple hops or intermediate nodes are involved in data transmission, such as in leased lines, virtual private networks (VPNs), and satellite communication.
      - 🥑**End-to-end encryption**: is a method of securing data such that it is encrypted on the sender’s device and only decrypted on the recipient’s device, ensuring that no one in between (including service providers, network administrators, or any other intermediary) can access or alter the data.
          - With E2EE, data is encrypted before it leaves the sender's device and remains encrypted as it travels across the network. It is only decrypted when it reaches the intended recipient. This means that even if the data is intercepted during transmission, it cannot be read by unauthorized parties.
          - Unlike link encryption, where data is decrypted and re-encrypted at each network hop, E2EE keeps the data encrypted throughout its entire journey. Intermediate nodes, like routers or servers, only see the encrypted data and cannot decrypt it.
          -  E2EE is particularly effective at protecting data from eavesdropping, as even if the communication is intercepted, the encryption prevents the contents from being read.
          -  End-to-end encryption is used in applications where data security and privacy are critical, such as in messaging apps (e.g., WhatsApp, Signal), secure emails, and some online banking systems.
...
- 📁**CPU**
    ...
    - 💥**The Trusted Computing Base (TCB)** is the total combination of protection mechanisms for a computer system, including hardware, software, and firmware. These components work together to form a trusted environment that enforces security by controlling access to critical data and processes.
        - The TCB includes the security kernel. The security kernel is the hardware, firmware, and software elements of the TCB that enforce the reference monitor.
            - The **reference monitor** mediates all access from all subjects to all objects. It must always be turned on, be verifiable, be tamperproof, as well as not be by-passable. The reference monitor is described as an abstract machine. The abstract machine means that the reference monitor should live as low as possible (closest to the hardware) and be as small as possible. As small as possible in terms of the number of lines of code it contains. The TCB and its elements can be traced back to 1972 and Mr. Anderson.
...
- **Different types of Memory**:
    ...
    - ❄️RAM: Random Access Memory (RAM) is computer memory used for data storage and quick access by the CPU. It's volatile, meaning data is lost when powered off. RAM's speed and capacity significantly impact system performance. RAM is volatile memory used to hold instructions and data of currently running programs. It loses integrity after loss of power.
        - **memory map**: A list of addressable Random Access Memory (RAM) locations can be referred to as a memory map. A memory map is a structure or diagram that details the specifics of how memory is organized and utilized within a computing environment. It shows the distribution and allocation of memory addresses to different types of data and hardware registers, facilitating efficient memory management.
        - 🎈SRAM: SRAM is fast, expensive memory that uses small latches called “flip-flops” to store bits. SRAM does not require refreshing and maintains integrity as long as power is supplied.
    ...
    - ❄️Firmware: Firmware stores programs that do not change frequently, such as a computer’s BIOS (discussed below) or a router’s operating system and saved configuration. Various types of ROM chips may store firmware, including programmable read-only memory (PROM), erasable programmable read-only memory (EPROM), and EEPROM, defined next.
         ...
         - 🎱EPROM may be “flashed,” or erased and written to multiple times.
             - 🎈UVEPROM chips may be erased with ultraviolet light.
             - 🎈 EEPROM chips may be erased with electrical current. A programmable logic device (PLD) is a field-programmable device, which means it is programmed after it leaves the factory. EPROMs, EEPROMs, and flash memory are examples of PLDs.
