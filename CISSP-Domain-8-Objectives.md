...
- 🐍**Data Warehouse**: collection of data sources such as separate internal databases to provide a broader base of info for analysis, trending and reference; may also involve databases from outside the org. A data warehouse is generally used for reporting and analysis across a large dataset from multiple sources. A data warehouse may need all the data converted into the same format before it can be analyzed.
    - A data warehouse is a large, integrated, and structured database that consolidates data from various sources within an organization. It serves as a centralized repository, optimized for data analysis and reporting. Data is extracted, transformed, and loaded into the warehouse, enabling businesses to access historical and real-time data for decision-making purposes. Its design allows for complex queries, analytics, and business intelligence, providing valuable insights into the organization's performance and trends.
- 🐍**Data Lake**: a data warehouse incorporating multiple types of streams of unstructured or semi-structured data. Data warehousing and data lakes have the purpose of having data that is analyzable. A data lake is the equivalent of a data warehouse but in the 📝big data world. Both data lakes and data warehouses are created for the specific purpose of analyzing data to gain insights into the business.
- 🐍**Data Mining**: analysis and decision-making technique that relies on extracting deeper meanings from many different instances and types of data; often applied to data warehouse content. Data mining is searching through data warehouses and correlating data. Analysts use data mining to find potential revenue. For instance, an analyst could data mine a product's sales patterns and identify that October is the best sales month. Marketing and sales could then increase during those months to increase revenue for the company.
- 🐍**Data sleuthing** involves investigative techniques to uncover hidden or valuable insights within datasets. It includes data analysis, pattern recognition, and exploratory methods to extract meaningful information for decision-making or research. Data surveying is the systematic collection, examination, and analysis of data to gain insights and understanding. It involves methods like surveys, questionnaires, or observations to gather and interpret information effectively. 
...
- 🐍**Database Management System** (DBMS) is a software suite used to manage sets of structured data within a database. A DBMS can be used to query or alter data and manage the security roles of a database. A DBMS acts as the middleware between the user and the database engine. There are many types of database engines. They include Structured Query Language (SQL), noSQL, mongo DataBase (mongoDB), and more. The DBMS controls access to the database.
    - A database is a 📝single collection of data or a 📝structured collection of data. Relational databases have data organized into tables. 
    - A security framework related to databases is the 📝Clark-Wilson (also known as the Chinese Wall) model. It has the intention of controlling access to the database to protect the mutual exclusivity of the company's customers.
- 🔴**Data Base Models**: The three most common types of databases today are relational databases, hierarchical databases, and object-oriented databases. Some database Models include
- ❄️**Object-oriented database model**: database model that uses object-oriented programming concepts like classes, instances, and objects to organize, structure, and store data and methods; schemas define the structure of the data, views specify table, rows, and columns that meet user/security requirements
- ❄️**Network database model**: database model in which data elements and records are arranged in arbitrary linked fashion (.e.g lists, clusters, or other network forms). The network model is not used much. The user can move too easily through the data, making it hard to control access.
- ❄️**Relational database model**: The relational model, introduced by 📝Edgar F. Codd, is the most popular general database model. Data elements and records arragned in tables which are related or linked to each other to implement business logic, where data records of different structures or types are needed together in the same activity. Records are identified using a variety of keys. A relational database is centralized, with all data stored in a single location (a server).
    - The relational model is similar to a spreadsheet and is a 📝one-to-one data model. In relational models, data is organized into tables. The entries/rows/records contain fields or attributes (columns). Then one record in one table can be linked to a record in another table.
        - 🎈Tuples (or records) represent the 📝rows of a table in a relational database. A tuple is an ordered list of elements. For instance, when querying a list of customers, each customer record with all of its attributes is called a tuple. A row, or one record is called a tuple.
        - 🎈Column in a relational databaseare vertical data lists and a tuple is one record.
    ...
    - Relational database transactions have four required characteristics: atomicity, consistency, isolation, and durability. Together, these attributes are known as the ACID model, which is a critical concept in the development of database management systems. - **ACID 🟡Test**: data integrity provided by means of enforcing atomicity, consistency, isolation, and durability policies
        - 💢**Atomicity**: Database transactions must be atomic—­that is, they must be an “all-­or-­nothing” affair. It guarantees that the data is trustworthy. If any part of the transaction fails, the entire transaction must be rolledback as if it never occurred. It states that if any part of the transaction fails, the entire transaction must be rolled back
        ...
        - 💢**Durability**: Database transactions must be durable. That is, once they are committed to the database, they must be 📝preserved. Databases ensure durability through the use of backup mechanisms, such as transaction logs. The durability principle says that transactions committed to the database must be preserved. Durability ensures that the changes will remain after a transaction is completed and cannot be rolled back.
...
- ❄️**Hierarchical Database Model** hierarchical database model organizes data in a tree-like structure. Data is arranged in a hierarchy of parent and child records, with each child having only one parent but each parent potentially having multiple children.
    ...
    - The hierarchical model is a 📝one-to-many data model and forms a logical tree structure. Corporate organization charts and NCAA March Madness brackets are common examples of the hierarchical model.
        ...
        - Several methodologies have emerged that take these Agile principles and define specific processes around them:
            - ❄️**Scrum**: a management framework that teams use to self-organize and work towards a common goal; it describes a set of meetings, tools, and roles for efficient project delivery, allowing teams to 📝self-manage, learn from experience, and adapt to change; named from the daily team meetings, called scrums
                - The 📝daily stand-up meeting is a hallmark of the Scrum methodology. The meetings are called scrums and are essential to the process. Scrum is a methodology that embraces the Agile philosophy, but not all Agile approaches use the daily scrum.
                    - 🎈The scrum master directs the development team toward their goals, interacts with stakeholders, and ensures a clear path for the team to complete their objectives. When the scrum master interacts with the stakeholders and learns the customer's needs, they will communicate that to the team. 
...
- 8.1.2 Maturity models (e.g., Capability Maturity Model (CMM), Software Assurance Maturity Model (SAMM))
    ...
    - It is a management process to foster the ongoing and continuous improvement of an org's processes and workflows for developing, maintaining and using software. The Software Capability Maturity Model (SCMM) is based on the principle that a mature software development process will produce quality software.
        ...
        - stages of the CMM:
            - 🆎Level 1: **Initial**: process is disorganized; usually little or no defined software development process. This is a 📝reactive and unpredictable stage. It indicates that work is getting finished, but often coming in over budget and late. 
            - 🆎Level 2: **Managed\Repeatable**: in this phase, basic lifecycle management processes are introduced. Reuse of code in an organized fashion begins, and repeatable results are expected from similar projects. The key process areas for this level include Requirements Management, Software Project Planning, Software Project Tracking and Oversight, Software Subcontract Management, Software Quality Assurance, and Software Configuration Management.
                - Once a company begins a structured formal process for software development, they are considered to be at the repeatable level.
            ...
            - 🆎Level 3: **Defined**: in this phase, software devs operate according to a set of 📝formal, 📝documented software development processes. The Defined stage of the SW-CMM is marked by the presence of basic 📝lifecycle management processes and reuse of code. It includes the use of requirements management, software project planning, quality assurance, and configuration management practices.
                - organizations with some basic steps in place for their software development would be considered defined. 
                ...
                - In this phase, the organization is being 📝proactive. There are standards across the organization that guide programs, portfolios and projects.
            - 🆎Level 4: **Quantitatively Managed**: in this phase, there is better management of the software process. The organization uses 📝quantitative 📝predicatable measures to gain a detailed understanding of the development process. Software Quality Management is a process that occurs during level 4, the Managed stage of the SW-CMM. These organizations are precise with measurements and know exactly what they need to thrive based on their data. 
                - Organizations taking a tailored approach to their own needs while implementing careful measures are considered quantitatively managed organizations or simply managed organizations. The closer an organization can get to precisely measuring practices and success via numbers versus a qualitative approach using descriptive words, the better. 
            ...
            - 🆎Level 5: **Optimizing**: in this phase continuous improvement occurs. Focus is on 📝continuous process improvement through innovation and optimization. The organization regularly identifies weaknesses and makes improvements to prevent process defects and enhance capabilities. An experienced organization taking a 📝proactive approach, using accurate measures, and leading the industry is known as an optimizing organization. Here, they are leaders of the industry, providing improvements to overall industry practices. 
...
- 🟢**A Gantt chart** is a type of bar chart that shows the interrelationships over time between projects and schedules. It provides a graphical illustration of a schedule that helps you plan, coordinate, and track specific tasks in a project. They are particularly useful when coordinating tasks that require the use of the same team members or other resources.
    ...
    - A Gantt chart is a type of bar chart which best shows the interrelationships between 📝projects and 📝tasks over time. 
...
- 🟢**Program Evaluation Review Technique (PERT)** is a project-­scheduling tool used to judge the size of a software product in development and calculate the standard deviation (SD) for risk assessment. PERT relates the estimated lowest possible size, the most likely size, and the highest possible size of each component. The PERT chart  shows the dependencies between different project tasks.
    ...
    - A Program Evaluation Review Technique (PERT) chart is similar to a critical path chart. PERT was created for the military and a critical path for the civilian world. The PERT chart shows three-time estimates. One for the shortest possible amount of time, the second for a probable amount of time, and the third for the longest amount of time tasks may take.
...
- 8.2.1 Programming languages
    ...
    - ⚒️**Machine language (1GL)**: machine language is a generation one software programming language, which is what a computer speaks. Computers only understand binary, also called machine language, which consists only of 0s and 1s. A sequence like 10110000 01100001 instructs the CPU to load a value into a register.
    - ⚒️**Assembly language (2GL)**: a way of using mnemonics\symbolic codes to represent the basic instruction set of a CPU. Assembly language falls under generation two of the software programming languages. This language is one level above machine language and uses assemblers to transition code into machine language.  Assembly is a very low-level language that requires intricate knowledge of the system’s architecture. Programs written in Assembly are hardware-specific and are not compatible between different Central Processing Unit (CPU) types. Assembly code: MOV AX, 1 (move the value 1 into register AX).
    ...
    - ⚒️**High-level\Compiled languages (3GL)** Third-generation programming languages, such as C/C++, Java, and Python, are known as high-level languages. Generation three is for high-level languages, such as C. Compilers take this language and transition it to machine language. Compiled languages use a compiler to generate an executable file. High-level languages are more abstract, closer to human language, and independent of machine architecture. 
        - high-level languages allow developers to write instructions that better approximate human communication.
        - uses a compiler to converts source code into machine-executable format
        ...
        - C, c++,  Erlang, Haskell, Rust, Go, and Fortran are all compiled programming languages. 
            - 🎈Compiler: converts source code into machine-executable format
            - 🎈Decompilers: convert binary executable back into source code
    - ⚒️**Interpreted language**: uses an interpreter to execute; sourcecode is viewable; e.g. Python, R, JavaScript, VBScript.
        ...
        -  An interpreted language is a programming language in which code is executed line by line by an interpreter at runtime, rather than being precompiled into machine code.  Interpreted languages can 📝belong to various generations (typically 3rd generation languages (3GL), such as Python or JavaScript). They are defined by the way they are executed, not by the level of abstraction.
    ...
    - ⚒️**Very High Level Language (4GL)**: Very high-level languages designed to be more abstract and closer to human-readable instructions, focusing on what needs to be done rather than how. They are often used for specific purposes like database management or report generation. Examples: SQL, MATLAB, Oracle Reports.
    - ⚒️**Natural Language (5GL)**: 5GLs are based on natural language processing and artificial intelligence, where the program can understand human language or logic-based queries. Examples: Prolog (used in AI applications), LISP (used in AI), and modern AI-driven languages. Real-World Use: Primarily used in artificial intelligence research, expert systems, and natural language processing (NLP).
...
- 8.2.2 🔴Libraries
    ...
    - Software libraries are groups of pre-created code by other developers that can be used to integrate into your code. This saves developers 📝time when creating a program by reusing code that has already been made by others.
        - 📝Risks: Security risks greatly increase with the use of 📝open-source software libraries, especially when the organization can't necessarily trust the software library or who developed it. Any outside code implemented into an organization's use can lack input validation or prior testing. Additionally, the software may be malicious and intend to provide your company's proprietary information or customer information to the creator.
    ...
    - Code libraries are packages of reusable functions that may be incorporated into individual development projects.
        ...
        - ⚒️**Code repositories** are collections of code made by many people, typically open for the use of others. An example of this would be 📝Github.  A code repository is a centralized storage system that enables developers to manage, 📝version, and 📝collaborate on software projects. It facilitates source code sharing, tracking changes, and maintaining project history. Through version control systems like Git, developers can contribute, merge changes, and resolve conflicts efficiently, ensuring a smooth development workflow. 📝Code repositories may be used to manage the distribution and updating of these libraries.
...
- 8.2.5 Runtime
    - 💥**RunTime Environments (RTE)**: allows the portable execution of code across different operating systems or platforms without recompiling (e.g. Java Virtual Manager (JVM))
        - this sometimes uses 📝portable code, which needs translation between each environment, the role of the RTE
            - 🎈**Portable code** is code that can function in any environment without a compiler. This code is typically found in a runtime environment. Specifically, it is the code providing the functionality of an operating system for a virtual machine, for example. These file types can often be seen as ".iso" images. Although it may appear similar to a simple template, portable code is not static. In other words, it interacts with the virtual machine it is joining and provides functionality.
            - Runtime refers to the period during which a program or application is executing or actively running on a computer. It starts when the program is launched and continues until it is terminated. 
...
- 8.4.1 Commercial-off-the-shelf (COTS): Software that is developed by a third party and typically sold for a profit is commonly described as Commercial Off The Shelf (COTS). Examples of well-known COTS applications include Microsoft Office, Intuit QuickBooks, and Adobe Creative Cloud.
    ...
    - Often, companies purchasing Commercial Off-The-Shelf (COTS) software open to the public will not receive the source code of their products. This prevents any organization from making changes or tailoring the software to their needs by editing the code. It also encourages future software sales because, if anyone can edit the software, it would be easier to find edits online versus buying the newest version.
    - A common security argument is that COTS software is 🚫less secure and less transparent than open-source products because the community cannot review the code and find the problems quicker, with the ability to modify it quickly. 
...
- 8.4.2 ❄️**Open source**
    - Open source is typically released with licensing allowing code access and inspection so devs can look for security issues
        ...
        - Open-source products can have a wide variety of integrations and many times can still integrate with proprietary items.
        - Additionally, they are often free or may have some paid features. Not because it is open source means it is always entirely free. Open-source does not mean that it is free or completely free.
        - 📝Android is an open-source operating system. It began in 2005 as an open-source project, was bought out two years later by Google, and has remained that way since. The organization may want to stay away from using Android as an operating system on any device if they have reason to believe vulnerabilities could become public knowledge much faster, for example.
- 8.4.3 Third-party
...
- 8.4.4 Managed services (e.g. Software as a Service (SaaS), Infrastructure as a Service (IaaS), Platform as a Service (PaaS))
    - As orgs continue to migrate to the cloud (SaaS, IaaS, PaaS), they should increase the security assessment of those services
        - 🎵Community cloud deployment model provides cloud-based assets to two or more organizations. It is a cloud application has been deployed and shared among several organisations with similar concerns. it is shared among a group of organizations with common interests and requirements. It is not available for public use but is used by a specific community.
        - 🎵Public cloud model includes assets available for any consumers to rent or lease. A Public Cloud is a cloud deployment model that is provisioned for open use by the 📝general public. Public clouds are operated by third-party cloud service providers and offer services and resources over the internet. These services are shared among multiple organizations or users and are accessible by anyone who subscribes to the service.
...
- 8.4.5 **Cloud Service Models**
    ...
    - 🔴**Platform as a Service (PaaS)**: Provides a platform allowing customers to develop, run, and manage applications without managing underlying infrastructure. Examples: AWS Elastic Beanstalk, Microsoft Azure App Service, Google App Engine.
        ...
        - Platform as a Service (PaaS) has two options, server-based and server-less.
            - 🔨Server-based virtual MacOS device platform for example.
            - 🔨Server-less is used to run an application or a function without the annoyance of dealing with the server operating system.
    ...
    - 🔴**Anything as a Service (XaaS)**:  Anything as a Service (XaaS) is a reference to anything provided by a cloud provider.  
...
- 8.5.2 Security of Application Programming Interfaces (APIs)
     - 🔴**Application Programming Interface (API)**: specifies the manner in which a software component interacts with other components
        ...
        - 🔨**Closed system** is designed to work well with a narrow range of other systems, generally all from the same manufacturer. The standards for closed systems are often proprietary and not normally disclosed. A closed system is one that uses largely proprietary or unpublished protocols and standards. Closed systems and proprietary systems use proprietary hardware or software. 
        - 🔨**Open systems** are designed using agreed-upon industry standards. Open systems are much easier to integrate with systems from different manufacturers that support the same standards or that use compatible application programming interfaces (APIs).  An open system uses components from various vendors, with nonproprietary hardware or software for the motherboard, memory, or Basic Input/Output System (BIOS). ⛔ This is different from Open Source, which is a form of software that makes source code publicly available.
...
- 📕**Failure Management**: The concept of a fail management begins with the definition of a secure state. This idea was introduced by David Bell and Leonard LaPadula in 1976. The secure state is basically the definition of how a corporation wants something to behave and that includes through failure. This is defined in the orange book as well as 🍎ISO 15408.
    ...
    - 🍆**Fail-Secure** (Synonym of Fail Closed): Maintains a secure state during a failure, prioritizing security over other concerns. e.g  Blue screen of death to ensure that security is not compromised during a failure. If the system fails as desired (or according to 📝policy), it will fail securely. An example of this would be a firewall failing and not allowing access to assets by default. Once the failure state is defined and put into the policy and supporting documents, an answer can change from fail-closed to fail-secure. Failing secure is failing according to policy. This is how the Trusted Computer System Evaluation Criteria (TCSEC) (a.k.a the Orange Book) as well as the International Standards Organization (ISO) 15408 define fail secure. This is also supported by the Bell-LaPadula security model. They define the ‘secure state’ as the one defined within the policy of the business.
    - 🍆**Fail-Closed**: Closes or shuts down access when a failure occurs, restricting entry or use. e.g a network firewall that blocks all traffic if it encounters a malfunction. Sacrifices availability for security. The fail closed approach prevents any activity from taking place during a system security failure and is the most 📝conservative approach to failure management. Fail-closed would be a bank vault automatically locking if a bank loses power, for example.
        - The fail-closed failure state puts the system into a high level of security and disables it entirely or shuts it down until an administrator has the chance to diagnose the issue and restore the device to regular operation. This prevents attackers from causing the system to fail and gaining access afterward. An example of fail-closed would be if a network switch blocked all devices from authenticating if the authentication server became unavailable.
        - If the question does not mention anything about the corporation's needs or policy, fail-closed will be a better answer.
    ...
    - 🍆**Fail-Soft**: Allows a system to continue operating in a reduced or limited capacity after a failure. e.g A computer that turns off non-critical services to keep essential services running after a hardware failure. Fail-soft refers to something still allowing for operation in a reduced capacity. One example of this could be a door that opens based on a sensor and makes a noise upon entry. The door's sensor fails, so the door has to be opened manually. However, it still makes a sound to audibly notify people of an entry. 
...
- 🔴**Common Web Application Vulnerabilities**
    - 🔥Cross-Site Scripting (XSS/CSS): XSS attacks involve injecting malicious scripts into web pages, which are then executed in the context of another user’s browser. These scripts can steal cookies, session tokens, or other sensitive information from users, manipulate the content of the web page, or perform actions on behalf of the user. Input validation is the most effective defense against cross-site scripting attacks. Escaping restricted characters/metacharacters is also a good defence against XSS attacks.
        - It is prevented by input sanitization, output encoding, and security measures.
        ...
        - XSS/CSS fit into the category of 📝client-side manipulation. XSS occurs when an HTML code on a webpage is altered. The altered code then causes something to be executed on the client side. The client side is where the user is, it is the desktop/laptop/tablet/phone, etc.
        ...
        - 📝script, using an alert() call, is commonly used to probe websites for cross-site scripting vulnerabilities. Attacks where the malicious user tricks the victim's web browser into executing a script through the use of a third-party site are known as cross-site scripting (XSS) attacks.
    ...
    - 🔥Cross-Site Request Forgery (CSRF or XSRF): CSRF attacks force authenticated users to execute unwanted actions on a web application in which they are currently authenticated. By 🧠social engineering/tricking a user into clicking a link or loading an image, an attacker can send unauthorized requests to the web application. Cross-­site request forgery (XSRF) attacks exploit authentication trust between browser tabs. It is an attack utilizing social engineering and a malicious Uniform Resource Locator (URL) link to take advantage of a victim's existing browser session with a web application. XSRF or (CSRF) attacks exploit the trust that sites have in a user's browser by attempting to force the submission of authenticated requests to third-party sites.
        - The primary target of an XSRF attack is the 📝server, using the user's credentials. The attacker aims to perform unauthorized actions, such as changing account details, transferring funds, or making purchases.  This attack is mitigated by tokens, referrer validation, and secure authentication mechanisms.
    ...
    - 🔥Session Hijacking: It's when an attacker steals or takes over a session (like a logged-in state) that belongs to another user. This can happen if the session ID (which identifies the user’s session) is intercepted or guessed by the attacker. Once hijacked, the attacker can pretend to be the user without needing their login credentials.Transport Layer Security 📝(TLS) provides the most effective defense against session hijack-ing because it encrypts all traffic between the client and server, preventing the attacker from stealing session credentials.
        - Session hijacking attacks attempt to steal 📝previously authenticated sessions but do not force the browser to submit requests. the attacker takes over a legitimate user's active session with a server by stealing or guessing the session token or session ID. This allows the attacker to impersonate the user and interact with the system as if they were the legitimate user. An attacker intercepts or guesses a session ID (often through methods like network sniffing, cross-site scripting (XSS), or predicting session tokens).
        - If the unencrypted communications between a website and its user were intercepted during the negotiation of an e-commerce transaction, the user is most at risk of a session hijacking attack. Session hijacking occurs when captured authentication details are utilized by a malicious actor to assume the identity, and act on behalf of, one of the parties in that session, to the other party.
    - 🔥Session sidejacking occurs when an attacker steals 📝credentials by sniffing the traffic of other users, who are also on the same network as the attacker. In this example, the session cookies derived from the initial authentication of the victim were stolen by the attacker and used for a malicious act. Session sidejacking refers to the use of packet sniffing (uses network sniffing tools like Wireshark or Firesheep) to monitor network traffic over 📝unsecured connections, to steal a user's session cookie or session token over an 📝unencrypted connection (typically via Wi-Fi or other unsecured networks). It is essentially network-based session hijacking, where the attacker intercepts data traveling between the user's device and the server.
...
- 🔴**Malware: (Malicious Software)** is a general term used to describe any software intentionally designed to cause damage to a computer, server, client, or computer network. It encompasses a wide range of malicious programs, including but not limited to:       
    - 🔥Viruses: Programs that replicate themselves and infect other files on a computer. Require 📝human intervention to replicate. Viruses cannot reproduce without a user interacting with it or the legitimate host application. The user must open the email, hover over the photo, click on the link, open the file, etc. depending on the exact type of virus. Viruses infect legitimate files or programs and use them to spread once the user interacts (normally). If something can infect and reproduce on its own it would be called a worm. 
...
access rights. A rootkit is used to achieve or maintain 📝elevated privileges on a victim’s host. Rootkits frequently masquerade as system-level services to help remain undetected. Rootkits often have kernel-level access and are very difficult to detect or remove.
    ...
    - 🔥Logic Bombs: Malicious code that lies dormant until certain conditions are met. Logic bombs wait until certain conditions are met before delivering their malicious payloads. Logic bombs are programs or code that execute when certain conditions are met. It is common for IT or development personnel to hide malicious programs somewhere in a computer network that executes if their user account is ever disabled.
...
- 🔴**Miscellenous Attacks**
    ...
    - 🔥**TOCTOU attack**: time of check vs time of use (TOCTOU) attack takes advantage of the time delay between a security check (such as authentication or authorization) being performed and actual use of the asset. Time-­of-­check to time-­of-­use (TOCTTOU) attacks exploit timing differences that lead to race conditions. TOC/TOU is a type of timing vulnerability that occurs when a program checks access permissions too far in advance of a resource request.
        - The attack generally follows these steps:
            - 🩹Time of Check (TOC): At a specific point in time, the system checks a condition or state to determine whether the user or process is authorized to perform a particular action or access a resource.
            - 🩹Time of Use (TOU): After the check, there is a time gap before the system actually uses the result of the check to grant access or perform the action.
            - 🩹Exploitation: An attacker takes advantage of this time gap by changing the system's state or conditions between the TOC and the TOU. They may modify permissions, resources, or data to gain unauthorized access or perform actions that they should not be allowed to execute. 
    ...
    - 🔥**Buffer overflow**: source code vulnerability allowing access to data locations outside of the storage space allocated to the buffer; can be triggered by attempting to input data larger than the size of the buffer. Input that is too large can “overflow” a data structure to affect other data stored in the computer’s memory. Buffer overflow attacks allow an attacker to modify the contents of a system's memory by writing beyond the space allocated for a variable.
        - A buffer overflow attack attempts to overflow a process's memory boundary, causing it to overwrite adjacent memory locations. This will cause a program or operating system to either crash or execute the arbitrary code that the attack added that overflowed into the next memory space. 
    ...
    - 🔥**Cold boot attack** is a type of physical attack on computer memory where an attacker restarts a computer and attempts to recover data from the system's RAM before it is completely erased. The attacker gains physical access to a computer and performs a cold boot by shutting down or restarting the machine. After the reboot, the attacker quickly removes the RAM modules from the system and places them in another machine. The key aspect of this attack is that the data in RAM does not disappear immediately upon power-off; instead, it remains for a short period (seconds to minutes), during which it can be extracted. The attacker then uses forensic tools to analyze the memory dump and extract sensitive information, such as encryption keys, passwords, and other data that was held in RAM.
    ...
    - 🔥**Credential stuffing** is a type of cyberattack where attackers use stolen username-password pairs, typically from previous data breaches, to try to gain unauthorized access to user accounts on other websites or services. This attack exploits the common habit of password reuse across multiple platforms.
- 🟦**Computing Resilience**:        
    ...
    - 🛠️Multithreading permits multiple concurrent tasks (i.e., threads) to be performed within a single 📝process. It allows multiple instruction sets to run in parallel under a single process. A dual-core single-processor system can operate on two threads at a time (one by each core). A single-core single processor system would be limited to one thread executing at a time. Multithreading is a programming concept where multiple threads, or lightweight processes, run concurrently within a single program. It enhances performance by allowing tasks to be executed independently, improving efficiency in parallel processing. 
        - Multithreading is the sharing of a single CPU between multiple tasks. 
        - 🩹A thread is an individual instruction set that must be worked on by the CPU. Threads can execute in parallel with other threads that are part of the same parent process. This is known as multithreading. Threads are dynamically built and destroyed by the parent process.
        - 🩹A process is a program loaded into memory. Most modern applications take advantage of multithreading. 
    - 🛠️Multiprocessing: In a multiprocessing environment, a multiprocessor computing system (that is, one with 📝more than one CPU) harnesses the power of more than one processor to complete the execution of a multithreaded application. Multiprocessing involves utilizing multiple processors or cores within a single computer or across a network to execute tasks concurrently. It enhances computational power, enabling faster execution and improved performance for complex tasks.
        - When multiple Central Processing Units (CPUs) are used in parallel, it is referred to as Multiprocessing. Multiprocessing is the parallel execution of instructions. If a computer has more than one CPU, it can be used in parallel to execute instructions. For example, a database server can use up to eight processors at the same time to improve performance for data queries.
    - 🛠️Multiprogramming is a computing technique where multiple programs are loaded into memory simultaneously, allowing the processor to switch between them swiftly, enhancing CPU utilization and overall system efficiency. Multiprogramming is the execution of two or more programs by a single computer. 
