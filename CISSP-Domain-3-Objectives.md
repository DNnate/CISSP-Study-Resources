...
- The important concepts:
    - ✴️**Target of Evaluation (TOE)** To perform an evaluation, you need to select the TOE (e.g. firewall or an anti-malware app). The Target of Evaluation (TOE) is the 📝product or system that is the subject of an evaluation.
    - ✴️**Protection Profile (PP)** The evaluation process will look at the PP, which is a document that outlines the security needs (customer 🔥"I wants"🔥); a vendor might use a specific protection profile for a particular solution. The Protection Profile (PP) identifies the 📝security requirements for a class of security devices
...
- 3.7.1 Brute force
    - 🍮**Brute force**: an attack that attempts every possible valid combination for a key or password
        ...
        - 📝Spraying attack is a type of brute force attack that introduces a delay between attempts (by looping through different accounts and/or systems) to avoid tripping account lockout controls. 
...
- 3.7.11 Pass the hash
    - 🍮**Pass the hash (PtH)**: a technique where an attacker captures a password hash (as opposed to the password characters) and then simply passes it through for authentication and potentially lateral access to other networked systems 
        - The attacker uses a stolen hashed password directly to authenticate without needing to know or crack the original plaintext password. In systems like Windows, when a user logs in, the password is hashed (e.g., using NTLM or LM hash) and stored or transmitted for authentication. If an attacker can gain access to this hash (via memory scraping, privilege escalation, or network sniffing), they can 📝use the hash as-is to impersonate the user and gain access to systems. They don’t need to recover the original password.
    ...
    - 🍮**Rainbow Table Attack:** This is different from PtH. The attacker uses a precomputed table of hashed passwords (a rainbow table) to 📝reverse the hash and find the original plaintext password. When a password is hashed, it produces a specific hash value. A rainbow table is a large precomputed table of hash values and their corresponding plaintext passwords. The attacker compares the stolen hash against the table. If a match is found in the rainbow table, the attacker can reverse the hash to recover the original password. Rainbow tables exploit unsalted hashes, meaning if two users have the same password, they will also have the same hash, making it easier to crack using precomputed tables.
- 3.7.12 🍮**Kerberos exploitation**
    - 🍓**Overpass the Hash**: alternative to the PtH attack, used when 🧠NTLM is disabled on the network (AKA pass the key). The Overpass the Hash attack is similar to Pass the Hash but involves Kerberos. Attackers use a compromised user's 📝NTLM password hash to request a Kerberos ticket from the KDC, allowing them to authenticate as the user. The attacker first obtains the NTLM hash of the user. The attacker then uses the NTLM hash to request a Kerberos TGT from the KDC (effectively bypassing the need for the actual password). With the TGT, the attacker can then request service tickets and access resources as the compromised user. 📝Defence: Implement multi-factor authentication (MFA
    - 🍓**Pass the Ticket**: in this attack, attackers attempt to harvest tickets held in the lsass.exe process and if succesful, is able to move laterally using authenticated credentials. In a Pass the Ticket attack, an attacker acquires a valid ticket-granting ticket (TGT) or TGS session key from a 📝compromised account or system and uses it to authenticate and impersonate a legitimate user. This allows the attacker to gain unauthorized access to other systems and move laterally within the network without the need for further authentication. It is a common technique used in advanced persistent threats (APTs) to maintain persistent access and expand control within a network. In a Pass the Ticket attack, attackers steal valid Kerberos tickets from a compromised system and use them to authenticate to services without needing the user's credentials or password. The attacker dumps the Kerberos tickets (TGTs or TGS tickets) from the memory of a compromised machine. These tickets are then used to authenticate to other services within the same Kerberos realm or domain. Hence, No password or hash is required; the tickets are used directly to gain access to network resources. 📝Defence: Regularly clear cached tickets on systems
    - 🍓**Silver Ticket**: a silver ticket uses the captured NTLM hash of a service account to create a ticket-granting service (TGS) ticket (the silver ticket grants the attacker all the privileges granted to the service account). Similar to the Golden Ticket attack but more limited in scope. It allows attackers to forge a Service Ticket 📝(TGS) rather than a TGT, providing access to specific services, like SQL Server, HTTP, or CIFS (file shares). The TGT isn't involved, meaning the KDC is not needed for this attack, making it harder to detect. 📝Defence: Use strong, regularly rotated passwords for service accounts
    - 🍓**Golden Ticket**: if an attacker obtains the hash of the Kerberos Key Distribution Center account (📝KRBTGT: the account that signs the TGTs), they can create tickets at will within Active Directory (this provides so much power it is referred to as having a golden ticket).  It allows an attacker to forge a Ticket Granting Ticket 📝(TGT) and impersonate any user, including Domain Admins, granting them unrestricted access to a domain: 📝Defence: Regularly rotating the KRBTGT password
    ...
    - 🍓**Kerberoasting**: kerberoasting collects encrypted ticket-granting service (TGS) tickets. Kerberoasting is an attack that targets service accounts in Active Directory. Attackers request TGS tickets for service accounts, extract these tickets, and attempt to brute-force the service account’s password offline. 📝Defence: Implement managed service accounts (MSAs) that automatically rotate passwords
- 3.7.13 Ransomeware
...
- 3.9.8 Fire prevention, detection, and suppression
    ...
    - Fire extinguisher classes:
        ...
        - **🧯Class D**: metal, combustible metal fires include lithium, calcium, magnesium, lutetium, and cerium,  🟡dilythium (Sodium Chloride (NaCl), Dry Powder Extinguishers)
...
- 🔴**Physical Security Controls**:The six common physical security control mechanisms is ✏️Deter (guard dogs, Lighting), ✏️Deny (mantrap, turnstile, gate), ✏️Detect (CCTV,motion sensor), ✏️Delay (locks), ✏️Determine, ✏️Decide. 
    ...
    - **Locks:**
         - 🔒A tumbler lock is a common type of lock mechanism found in many doors and padlocks. It consists of a series of spring-loaded pins (tumblers) that align when the correct key is inserted, allowing the lock to turn and open. Incorrect keys won't align the tumblers, preventing the lock from turning. Tumbler locks offer basic security but are vulnerable to 📝lock-picking/📝bumping techniques. A tumbler lock is properly opened using its associated key which sets the lock pins into the correct position. 
             - 🚬Bumping: Tumbler lock are vulnerable to bumping. By inserting and agitating a special "bump key", the pins can be forced into the correct position, allowing the cylinder to turn.
             - 🚬Picking is the process of manipulating the cylinder pins to allow the attacker to open the lock. Picking exploits mechanical imperfections that allow the attacker to set each pin one at a time by applying tension to the cylinder.
             - 🚬Shimming is wedging material between the shackle and the locking mechanism.
             - 🚬Impressioning is re-creating a key by analyzing the bite marks left by a lock on a blank key.
             - 🚬Brute force is applying enough tension to the cylinder that it causes the pins to shear.
         ...
         - 🔒A card reader is a form of electronic access control lock that is unlocked using a badge or smart card associated with a specific individual, thereby supporting access by different individuals and reporting on specific individuals. It can report access by specific individuals
         ...
         - 🔒A cipher lock is a type of mechanical or electronic lock that requires users to enter a combination or code to unlock it, rather than using a traditional key. A cipher lock is characterized by a keypad, requiring a specific numerical sequence on the keypad to unlock an entrance. Keypads are used in data centers or even within restricted areas to add an extra level of security. Cipher locks are commonly used to control access to secure areas, such as offices, server rooms, or other restricted locations.
             - 🥑Mechanical Cipher Locks: These locks are purely mechanical, with no electronics involved. They use physical buttons that, when pressed in the correct sequence, allow the door to be opened.
                 - 🎈A combination lock uses numbered tumblers and dials that must be turned clockwise and counter-clockwise to preset numbers to open the device.
             - 🥑Electronic Cipher Locks: These locks use an electronic keypad and may include additional features such as keycards, biometrics, or logging of access attempts
                 - 🎈A smart lock uses a set personal code to enter and lock a facility where each individual has their own code.
                 - 🎈Keypad and pushbutton locks use a keypad or pushbutton for access.
...
- 🔴**Technical Physical Controls**: Technical controls involve code, hardware, firmware, etc.
     ...
     - 🦁**Cameras**: Can be overt and obvious in order to provide a deterrent benefit, or hidden and concealed in order to primarily provide a detective benefit. Vary in type, including visible light, infrared, and motion-triggered recording. Some cameras are fixed, whereas others support remote control of automated pan, tilt, and zoom (PTZ).
         ...
         - 🏷️**Closed-circuit television (CCTV)** systems act as a secondary verification mechanism for physical presence because they allow security officials to view the interior of the facility when a motion alarm sounds to determine the current occupants and their activities.
         - 🏷️**Enhanced video surveillance (EVS)**: systems are capable of object detection, which can include faces, devices, and weapons. Some can even automatically identify individuals and track their motion (gait analysis).  EVSs can be enhanced using machine learning to improve video monitoring through automation, improved image recognition, and pattern/activity interpretation
     ...
     - **Fire Detectors**: There are many types of fire detection systems:
         - 🚒Fixed-temperature detection systems: trigger suppression when a specific 📝temperature is reached. inexpensive and is the most common with potentially visible sprinkler head which serves as both the detection and release mechanism (plastic that melts at a specific temperature or small glass vial containing chemicals that vaporize to over-pressurize and shatter the container at a specific temperature to trigger release). This system is inexpensive and very reliable
             - Heat detection systems react to the significant changes in 📝temperature that are caused by a fire. They can prevent fires before they occur by detecting rising heat levels. Heat and humidity detectors are used together to prevent environmental damage in a data center.
         ...
         - 🚒Ionization systems uses an ionization chamber and a source of ionizing radiation to detect smoke.
...
-  🔴**Output Controls**: Output controls are designed to manage and protect data output from information systems. They ensure that printed or electronic output is properly handled and reaches the intended recipients while preventing unauthorized access or misuse.
- 🔴**Preventive Access controls** Preventive access controls attempt to stop or interrupt unauthorized attempts to gain access. Examples of preventive controls include Intrusion Prevention Systems (IPS), firewalls, encryption, access controls, fences, locks, biometrics, mantraps, lighting, alarm systems, separation of duties policies, encryption, auditing, and others. A smart card is an example of a preventive access control because it prevents unauthorized activity. 
- 🔴**Detective Access controls** A detective control discovers and alerts the operations center to a possible attack. Logs and the Security Information Event Manager (SIEM) are good examples of detective controls. Detective controls are deployed to detect or identify unwanted or unauthorized activity. These controls are frequently deployed after an event to ensure that future occurrences are discovered. Detective controls do not directly block or prevent unwanted activity. Detective controls include security cameras, motion detectors, and virus scans. An example would be an Intrusion Detection System (IDS), logs, cameras with motion detection, and more.
- 🔴**Deterrent Access control** is something that, when it is known by the would-be attacker, they decide not to do their bad deed as they are deterred from that behavior. An armed guard deters someone from breaking into a building. Or a policy that when violated could cost someone their job is also a deterrent. Or knowing that there is a camera and they are being monitored might dissuade someone from a bad activity.
...
- 🔴**SYMMETRIC**
    ...
    - 💊DES - Block Cipher, 📝Bock Size 64, 📝Key Size 56. It is both substitution and transposition. Data Encryption Standard (DES) was published in 1977 as a standard cryptosystem for government communications. DES has a key size of 64 bits; however, 8 bits are used for parity, giving DES an effective key length of 56 bits. DES has a block size of 64 bits, and each block is put through 16 rounds of encryption functions.
        - ⚒️DESX (Data Encryption Standard Extended) is an enhancement of the original DES algorithm. In DESX, the plaintext is first XORed with an additional 64 bits of key material before being encrypted using the DES algorithm. After the DES encryption, the output is XORed with another 64 bits of key material. This added key material before and after the DES encryption step provides additional security by increasing the complexity of the encryption process. 
    - 💊2DES - Block Cipher (vulnerable to meet-in-the-middle attack)
    - 💊3DES - Block Cipher, 📝Bock Size 64, 📝Key Size 112, 168. Triple Data Encryption Standard (3DES) has four different modes. 
        - 🛠️E(K1 ,E(K2 ,E(K3 ,P))) DES-EEE3 encrypts the data three times using three different keys.
        - 🛠️E(K1 ,D(K2 ,E(K3 ,P))) DES-EDE3 encrypts the data twice with decryption in the middle and uses three different keys.
        - 🛠️E(K1 ,E(K2 ,E(K1 ,P))) DES-EEE2 encrypts the data three times using two different keys.
        - 🛠️E(K1 ,D(K2 ,E(K1 ,P))) DES-EDE2 encrypts the data twice with decryption in the middle and uses two different keys. 
...
- The TEMPEST program creates technology that is not susceptible to Van Eck phreaking attacks because it reduces or suppresses natural electromagnetic emanations. TEMPEST is a US and UK government term referring to the study of electromagnetic emissions that can be intercepted by a system attacker and then controlled. The emissions are intercepted from a distance because devices emit electromagnetic frequencies as they transmit data. Shielded cabling reduces the risk of attackers intercepting electronic emissions because it stops the "leakage" of electromagnetic impulses.
- TEMPEST eavesdropping or Van Eck phreaking countermeasures include the following:
    - 🐌Faraday Cage: A Faraday cage is an enclosure that blocks or absorbs electromagnetic fields or signals. Faraday cage containers, computer cases, rack-­mount systems, rooms, or even building materials are used to create a blockage against the transmission of data, information, metadata, or other emanations from computers and other electronics. Devices inside a Faraday cage can use EM fields for communications, such as wireless or Bluetooth, but devices outside of the cage will not be able to eavesdrop on the signals of the systems within the cage.
        - Faraday cages are used as absorbing capacitors to prevent the release of electromagnetic signals. Emanation cage have the same application as a Faraday cage.  
    - 🐌White Noise:  White noise generates false emanations that effectively "jam" the true emanations from electronic equipment. It is a physical security control that broadcasts false emanations constantly to mask the presence of true electromagnetic emanations from computing equipment.
        - White noise is false traffic used to mask or hide the presence of real traffic. White noise includes a real signal mixed in with false information. It can be continuously transmitted or at a specific variable length. 
    - 🐌Control Zone: A control zone is an area that is monitored by physical security.
...
- 🟢**Mobile Device Management (MDM) Concepts**:  Security. 🍎NIST SP 800-124 is a guideline for managing the security of mobile devices in the enterprise.
  - ❄️Mobile Device Management (MDM) are systems used to manage and secure endpoints within an organization. MDM primarily focused on managing mobile devices such as smartphones and tablets and supported Devices generally includes mobile operating systems like iOS, Android, and Windows Mobile.
      - Mobile Device Management (MDM) is a software solution that helps with the challenging task of managing a myriad of mobile devices that employees use to access company resources. The goals of MDM are to protect the organization's data, provide monitoring, enable remote management, and support troubleshooting. 
